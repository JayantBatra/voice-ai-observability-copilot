...
export function verifyWebhook(rawBody, headers) {
  ...
  const workflowSig = headers["x-webhook-signature"];
  const legacySig = headers["x-wh-signature"];
  if (process.env.HL_WEBHOOK_SECRET && (ghlSig || workflowSig)) {
    const sig = workflowSig || ghlSig;
    return { ok: verifyHmac(rawBody, sig), reason: "hmac-sha256" };
  }
