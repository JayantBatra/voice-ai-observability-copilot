...
| Real-time transcript ingestion | `POST /webhooks/ghl` or `/webhooks/hl` in `server.js`, signature verification in `lib/webhook.js` |
...
| App → HighLevel write-back | auto contact tags/notes in `lib/orchestrator.js`; manual prompt/contact actions in `server.js` + `public/app.js` |
...
GHL Call Log API ─► backfill ─────┘       ├─► analysis.js (ranked recs; optional LLM)
                                          └─► orchestrator.js (tags + contact notes)
                                                         │
                  Vue 3 dashboard (Custom Page) ◄── JSON API (server.js)
                              └── manual actions ──► HL tags/notes/agent prompt
```
...
- **`lib/orchestrator.js`** — safe bidirectional loop: auto-tags contacts and adds analysis notes for live HighLevel calls.
...
2. **Register the app:** Marketplace Portal → **My Apps** → your app, with **App Type = Sub-Account**.
   ...
   - Scopes: Voice AI **agents** read/write, **call logs** read, and **contacts** read/write (set while the app is in draft).
   - Webhooks → subscribe to the Voice AI call/transcript events, URL `https://<host>/webhooks/ghl` for Marketplace webhooks or `https://<host>/webhooks/hl` for Settings → Integrations webhooks.
...
| Workflow webhook `/webhooks/hl-workflow` | **Functional** for workflow feedback events such as `needs-human-followup` tag triggers |
| Auto contact tags + analysis notes | **Functional** for live HighLevel calls with `locationId` + `contactId`; skipped in fixture mode |
| Manual dashboard write-back | **Functional** for live calls/agents: tag contact, add note, apply prompt fix |
...
lib/orchestrator.js     auto contact tags + contact notes after scoring
...
scripts/send-webhook.mjs no-dependency HighLevel webhook test payload
